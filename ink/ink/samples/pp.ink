...
{cons, nil, text, line, nest, layout, group, pretty} := (
  ...
  {reduce, map} := import('functional.ink')
...
{flatten, map} := import('functional.ink')
...
fillwords := s => folddoc((x, y) => cons([x, group(line), y]), map(text, (s, _) => words(s)))
fill := xs => len(xs) :: {
  0 -> nil
  1 -> xs.0
  _ -> union(
...
(
  ...
  showFill := (f, xs) => len(xs) :: {
    0 -> nil
    _ -> bracket('', dbg(fill(dbg(concat(map(xs, (x, _) => f(x)))))), '')
  }
  showXMLs := x => x.kind :: {
    'elt' -> len(x.c) :: {
