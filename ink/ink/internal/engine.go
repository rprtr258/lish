...
func trampoline(thunk ValueThunk) Value {
	for {
		th := thunk()
		if v, ok := th.(ValueThunk); ok {
			thunk = v
		} else {
			return th
		}
...
func (ctx *Context) Eval(node NodeID) (val Value, err *Err) {
	...
	kont := func(v Value) ValueThunk { return func() Value { return v } }
	thunk := ctx.Engine.AST.Nodes[node].Eval(ctx.Scope, ctx.Engine.AST, kont)
	if val = trampoline(thunk); isErr(val) {
		err = val.(ValueError).Err
		LogError(err)
	}
