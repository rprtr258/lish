...
bmp := import('vendor/bmp')
doubleDigit := import('lib/util').doubleDigit
vec3 := import('lib/vec3')
v := vec3.create
vnorm := vec3.norm
vadd := vec3.add
vmul := vec3.multiply
vlist := vec3.list
ray := import('lib/ray')
shape := import('lib/shape')
sphere := shape.sphere
camera := import('lib/camera')
material := import('lib/material')
...
data := map(range(0, Width * Height, 1), i => (
  ...
  x :: {
    0 -> (
      ...
      log(f(
        'Rendering row {{0}} -> {{1}}px/sec. {{2}} remaining.'
        [
          y
          floor(speed)
          f('{{0}}:{{1}}', [
