...
split := str.split
trim := str.trim
sortBy := import('https://gist.githubusercontent.com/rprtr258/e208d8a04f3c9a22b79445d4e632fe98/raw/quicksort.ink').sortBy
escapeHTML := import('../vendor/escape.ink').html
Reader := import('../vendor/reader.ink').Reader
...
parseDirective := directive => (
  parts := pipe(directive, [
    s => split(s, ' ')
    s => map(s, s => trim(s, ' '))
    s => filter(s, s => len(s) > 0)
  ])
  parts.0 :: {
    'if' -> {
      type: Directive.If
...
compileReader := reader => (
  ...
  (sub := () => c := next() :: {
    () -> pipe(parts, [
      xs => filter(xs, s => len(s) > 0)
      xs => map(xs, part => type(part) :: {
        'string' -> {
...
generateDirective := (reader, params) => (
  ...
  directive.type :: {
    ...
    Directive.Each -> (
      ...
      sortByFunc := (sortKey := directive.by :: {
        ` by default, things (usually pages) are sorted by path `
        () -> item => item.path
        _ -> item => res := resolveParamValue(sortKey, item) :: {
          () -> ''
          _ -> res
        }
      })
      values := sortBy(values, sortByFunc)
