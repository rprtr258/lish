...
withParts := cb => dir(TplDir + '/parts', evt => evt.type :: {
  ...
  _ -> (
    files := evt.data
    compiled := {}
    peach(files, entry => readFile(TplDir + '/parts/' + entry.name, file => (
      file :: {
